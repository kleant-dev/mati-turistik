...
import { updateBookingSettings } from "../actions/updateBookingSettings";
...
export const AdminSettings = ({
  ...
  const form = useForm<z.infer<typeof AdminSettingsFormSchema>>({
    ...
    defaultValues: {
      maxDays: settings.maxBookingDuration.toString(),
  ...
  function onSubmit(values: z.infer<typeof AdminSettingsFormSchema>) {
    startTransition(async () => {
      await updateBookingSettings(values).then((data) => {
        if (data?.error) {
          setError(data.error);
        }
        if (data?.success) {
